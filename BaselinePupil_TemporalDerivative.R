...
##### ANALYSIS - with PS and AS results

df_TD_oculo <- read.csv("TD_oculomotor.csv", header = TRUE, quote="\"", stringsAsFactors= TRUE, strip.white = TRUE, na.strings=c("NA", "-", "?"))
attach(df_TD_oculo)

df_TD_oculo <- subset(df_TD_oculo, select = -c(X))
...
levels(df_TD_oculo$condition)
df_TD_oculo$condition <- factor(df_TD_oculo$condition,levels(df_TD_oculo$condition)[c(2,3,1)])
levels(df_TD_oculo$condition)


#temp derivative vs correct AS
plot_TD <- ggplot(df_TD_oculo, aes(tempdiff, correct_AS, colour = condition)) + geom_point() 

plot_TD + scale_colour_manual(values = c("#f3a953", "#3e64ff", "#ff5151"))


#temp derivative vs PS latency
plot_TD2 <- ggplot(df_TD_oculo, aes(tempdiff, PS_latency, colour = condition)) + geom_point() 

plot_TD2 + scale_colour_manual(values = c("#f3a953", "#3e64ff", "#ff5151"))
